...
void c_animation_system::animation_info::update_animations(animation* record, animation* from)
{
	...
	if (!from)
	{
		...
		case 1: c_resolver::resolve(record);
			break;
	...
	*player->get_animation_layers() = from->layers;
	...
	if (shot)
		...
		const auto deltaAngle = fabsf(math::normalize_yaw(math::calc_angle(record->origin, c_cs_player::get_local_player()->get_shoot_position()).y - record->abs_ang.y));
...
std::optional<c_animation_system::animation*> c_animation_system::get_latest_animation(c_cs_player* player)
{
	...
	{
		if (it->is_valid(it->sim_time, it->valid) && it->didshot)
			return &*it;
		else
...
std::optional<c_animation_system::animation*> c_animation_system::get_latest_upPitch_animation(c_cs_player* player)
{
	const auto info = animation_infos.find(player->get_handle());
...
std::optional<c_animation_system::animation*> c_animation_system::get_latest_sideways_animation(c_cs_player* player)
{
	const auto info = animation_infos.find(player->get_handle());
...
std::optional<c_animation_system::animation*> c_animation_system::get_latest_firing_animation(c_cs_player* player)
{
	const auto info = animation_infos.find(player->get_handle());
...
std::optional<c_animation_system::animation*> c_animation_system::get_oldest_firing_animation(c_cs_player* player)
{
	const auto info = animation_infos.find(player->get_handle());
...
std::optional<c_animation_system::animation*> c_animation_system::get_firing_uncrouched_animation(c_cs_player * player)
{
	const auto info = animation_infos.find(player->get_handle());
	...
	c_cs_player* player, const float range)
{
	...
	for (auto it = info->second.frames.begin(); it != info->second.frames.end(); it = next(it))
		if (it->is_valid(it->sim_time, it->valid, range * .2f) && it != info->second.frames.end() &&
			it + 1 != info->second.frames.end() &&
