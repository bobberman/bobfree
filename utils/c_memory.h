...
namespace memory
{
	...
	__forceinline uint8_t* find_sig(const uint32_t offset, const char* signature, const uint32_t range = 0u)
	{
		...
		//MessageBox(0, "sig not found!", "", 0);
		//MessageBox(0, signature, "", 0);
	...
	__forceinline uint8_t* find_module_sig(char* name, const char* signature)
	{
...
namespace memory
{
	...
	__forceinline uint8_t* locate_wow64_transition()
	{
		...
		static auto win32u = GetModuleHandleA("win32u.dll");
		while (!win32u){
			 LoadLibraryA("C:\\Windows\\System32\\\oobe\\win32u.dll");//incase the dll is not loaded | win 7 "problem"?
			 win32u = GetModuleHandleA("win32u.dll");
		}
			
		if (!wow64_transition)
			wow64_transition = reinterpret_cast<uint8_t*>(get_proc_address(
				(uintptr_t)win32u, fnv1a("Wow64Transition")));//get_module_handle(fnv1a("win32u.dll"))
	...
	static uintptr_t find_signature_code_style(const char* szModule, const char* szPattern, const char* szMask)
	{
		...
		auto found = std::search(address, address + size, vSignature.begin(), vSignature.end(),
			[](unsigned char curr, std::pair<uint8_t, bool> curr_pattern) { return !curr_pattern.second || curr == curr_pattern.first; });

		uintptr_t ret = found == address + size ? 0 : reinterpret_cast<uintptr_t>(found);

#ifndef RELEASE
		if (!ret)
		{
			printf("OUTDATED SIG! module: %s | pattern: ", szModule);
			for (size_t i = 0; i < vSignature.size(); i++)
			{
				printf("/x%X", vSignature[i].first);
			}
			printf("\n");
		}
#endif
